...
\maketitle
\begin{abstract}
This is a derivative of spc003, part of a suite of documents
giving a formal specification of the HOL logic,
and is an early part of an enterprise
which involves the design of a new logical kernel
for a new kind of proof technology.
Part of the aim is a system which is well versed in its own
metatheory.
\end{abstract}
\vfill           
\begin{titlepage}
...
Derived from \cite{DS/FMU/IED/SPC003} and at present depending
on the theory spc001 from \cite{DS/FMU/IED/SPC001}.

spc003 defines the primitive inference rules,
including conservative extension mechanisms in a manner
which is not as transparently executable as it might have been.
for reasons which we need not go into.

This document is derived from it for reasons which demand that
``contructive'' approach despite any awkwardness in that approach,
by which I mean that,
where in the original a constructor is defined as a relation
rather than as a function (possibly because native support
for partial functions in HOL is lacking),
in this version it will be defined as a total function
which yields a conventional default for values outside
its intended domain.
...
├
	∀v b⦁	mk_abs (v, b) = εt⦁ has_mk_abs((v,b),t)
■

ⓈHOLCONST
	⦏mk_eq⦎ : (TERM × TERM) → TERM
├
	∀ lhs rhs⦁ mk_eq(lhs, rhs) ⇔
	mk_comb(mk_comb (equality_ty(type_of lhs), lhs),rhs)
■
...
├
	∀ lhs rhs ⦁ mk_imp(lhs, rhs) = mk_comb(mk_comb(Implication, lhs), rhs)
■
...
and a set of some of
its free variables, one for each equation.
...
may need renaming will be
its binding occurrence in
a $λ-abstraction$.
ⓈHOLCONST
...
list is a valid proof (of the sequent at
its
